...
function nacs_start () {
	...
	__nest__ (
		...
		'nacs.admin', {
			__all__: {
				__inited__: false,
				__init__: function (__all__) {
					var __name__ = 'nacs.admin';
					var uriAdmin = __init__ (__world__.nacs.uris).uriAdmin;
					var uriThreadSetting = __init__ (__world__.nacs.uris).uriThreadSetting;
					var EventDispatcher = __init__ (__world__.nacs.util).EventDispatcher;
					var AdminArea = __class__ ('AdminArea', [object], {
						__module__: __name__,
						get __init__ () {return __get__ (this, function (self, area, authenticate) {
						...
						get __bindEvents () {return __get__ (this, function (self) {
							...
							self.database.ref (uriThreadSetting (self.dataId)).on ('value', self.onThreadSettingUpdated);
							self.select ().find ('button[name="enable-comment"]').click (self.onEnableComment);
							self.select ().find ('button[name="disable-comment"]').click (self.onDisableComment);
						});},
						get __toggleVisibility () {return __get__ (this, function (self, visible) {
							if (visible) {
						...
						get __getAdminUID () {return __get__ (this, async function (self) {
							try {
								var admin = await self.database.ref (uriAdmin ()).once ('value');
							}
							catch (__except0__) {
								return null;
							}
						...
						get __updateThreadSetting () {return __get__ (this, async function (self, readonly) {
							var updates = dict ({});
						...
						get onThreadSettingUpdated () {return __get__ (this, async function (self, py_update) {
							var val = py_update.val ();
							console.log ('Setting for this thread:', val);
							var readonly = true != val;
							self.eventReadonly.call (readonly);
							self.select ().find ('.readonly').toggle (readonly);
							self.select ().find ('.not-readonly').toggle (!(readonly));
						});},
						get onAuthenticateLogin () {return __get__ (this, async function (self) {
							var admin = await self.__getAdminUID ();
						...
						get onAuthenticateLogout () {return __get__ (this, async function (self) {
							self.__toggleVisibility (false);
						});},
						get onDisableComment () {return __get__ (this, async function (self) {
							console.log ('Disable commenting');
							await self.__updateThreadSetting (false);
						});},
						get onEnableComment () {return __get__ (this, async function (self) {
					...
					'</use>')
					__pragma__ ('<all>')
						__all__.AdminArea = AdminArea;
						__all__.EventDispatcher = EventDispatcher;
						__all__.__name__ = __name__;
						__all__.uriAdmin = uriAdmin;
						__all__.uriThreadSetting = uriThreadSetting;
					__pragma__ ('</all>')
				}
			}
		}
	...
	__nest__ (
		...
		'nacs.authenticate', {
			__all__: {
				__inited__: false,
				__init__: function (__all__) {
					var __name__ = 'nacs.authenticate';
					var EventDispatcher = __init__ (__world__.nacs.util).EventDispatcher;
	...
	__nest__ (
		...
		'nacs.comment', {
			__all__: {
				__inited__: false,
				__init__: function (__all__) {
					var __name__ = 'nacs.comment';
					var ComposeArea = __init__ (__world__.nacs.compose).ComposeArea;
					var uriThread = __init__ (__world__.nacs.uris).uriThread;
					var CommentArea = __class__ ('CommentArea', [object], {
						__module__: __name__,
						get __init__ () {return __get__ (this, function (self, dataId) {
							self.select = (function __lambda__ () {
								return $ (('.nacs-comment-area[data-id="' + dataId) + '"]');
							});
							self.dataId = dataId;
							self.database = firebase.database ();
							self.__initializeUI ();
						...
						get __addComments () {return __get__ (this, function (self, comments) {
							var target = self.select ().find ('[name="comments"]').empty ();
							var comments = sorted (comments, __kwargtrans__ ({key: (function __lambda__ (e) {
								return e ['timestamp'];
							})}));
							var __iterable0__ = comments;
							for (var __index0__ = 0; __index0__ < len (__iterable0__); __index0__++) {
								var comment = __iterable0__ [__index0__];
								var obj = $ ('<div class="nacs-comment">').html ('\n                <div class="meta">\n                    &lt;<span class="email"></span>&gt;\n                    <span class="timestamp"></span>\n                </div>\n                <textarea class="content" readonly></textarea>\n            ');
								obj.find ('.email').text (comment ['email']);
								obj.find ('.timestamp').text (new Date (comment ['timestamp']).toString ());
								obj.find ('.content').val (comment ['content']);
								target.append (obj);
							}
						});},
						get onComments () {return __get__ (this, async function (self, snapshot) {
							var comments = snapshot.val ();
							if (!(comments)) {
								self.select ().find ('[name="comments"]').empty ();
							}
							else {
								self.__addComments ((function () {
									var __accu0__ = [];
									for (var i in comments) {
										__accu0__.append (comments [i]);
									}
									return __accu0__;
								}) ());
							}
					...
					'</use>')
					__pragma__ ('<all>')
						__all__.CommentArea = CommentArea;
						__all__.ComposeArea = ComposeArea;
						__all__.__name__ = __name__;
						__all__.uriThread = uriThread;
					__pragma__ ('</all>')
				}
			}
		}
	...
	__nest__ (
		...
		'nacs.compose', {
			__all__: {
				__inited__: false,
				__init__: function (__all__) {
					var __name__ = 'nacs.compose';
					var uriThread = __init__ (__world__.nacs.uris).uriThread;
					var uriNewComment = __init__ (__world__.nacs.uris).uriNewComment;
					var AuthenticationArea = __init__ (__world__.nacs.authenticate).AuthenticationArea;
					var AdminArea = __init__ (__world__.nacs.admin).AdminArea;
					var ComposeArea = __class__ ('ComposeArea', [object], {
						__module__: __name__,
						get __init__ () {return __get__ (this, function (self, area) {
							self.select = (function __lambda__ () {
								return $ (area);
						...
						get __initializeUI () {return __get__ (this, function (self) {
							...
							self.login = AuthenticationArea (self.select ().find ('[name="login-area"]'));
							self.admin = AdminArea (self.select ().find ('[name="admin-area"]'), self.login);
							self.textarea = (function __lambda__ () {
								return self.select ().find ('textarea[name="nacs-content"]');
							});
						});},
						get __bindEvents () {return __get__ (this, function (self) {
							self.select ().find ('[name="composer-area"] button[name="submit"]').click (self.onSubmit);
							self.admin.eventReadonly.append (self.onReadonly);
							self.login.eventLogin.append ((function __lambda__ () {
								return self.onAuthChanged (true);
							}));
							self.login.eventLogout.append ((function __lambda__ () {
								return self.onAuthChanged (false);
							}));
						});},
						get onSubmit () {return __get__ (this, async function (self) {
							var user = self.login.getCurrentUser ();
							if (!(user)) {
								self.login.resetUI ();
								return false;
							}
							var text = str (self.textarea ().val ()).strip ();
							var dataId = self.select ().parents ('[data-id]').attr ('data-id');
							if (len (text) < 3) {
								alert ('Comment too short.');
								return false;
							}
							var updates = dict ({});
							var newKey = firebase.database ().ref ().child (uriThread (dataId)).push ().key;
							updates [uriNewComment (dataId, newKey)] = dict ({'uid': user.uid, 'email': user.email, 'content': text, 'timestamp': firebase.database.ServerValue.TIMESTAMP});
							console.log (updates);
						...
						get onReadonly () {return __get__ (this, async function (self, readonly) {
							self.select ().find ('[name="composer-area"]').toggle (!(readonly));
						});},
						get onAuthChanged () {return __get__ (this, async function (self, login) {
							self.select ().find ('[name="composer-area"] button[name="submit"]').attr ('disabled', !(login));
							self.select ().find ('[name="composer-area"] .not-logged-in').toggle (!(login));
						});}
					});
					__pragma__ ('<use>' +
						'nacs.admin' +
					...
					__pragma__ ('<all>')
						...
						__all__.ComposeArea = ComposeArea;
						__all__.__name__ = __name__;
						__all__.uriNewComment = uriNewComment;
						__all__.uriThread = uriThread;
					__pragma__ ('</all>')
				}
			}
	...
	__nest__ (
		...
		'nacs.count', {
			__all__: {
				__inited__: false,
				__init__: function (__all__) {
					var __name__ = 'nacs.count';
					var uriThread = __init__ (__world__.nacs.uris).uriThread;
					var CommentCount = __class__ ('CommentCount', [object], {
						__module__: __name__,
						get __init__ () {return __get__ (this, function (self, dataId) {
							self.select = (function __lambda__ () {
								return $ (('.nacs-comment-count[data-id="' + dataId) + '"]');
							});
							self.dataId = dataId;
							self.database = firebase.database ();
							self.updateCount (0);
						...
						get updateCount () {return __get__ (this, function (self, count) {
					...
					'</use>')
					__pragma__ ('<all>')
						__all__.CommentCount = CommentCount;
						__all__.__name__ = __name__;
						__all__.uriThread = uriThread;
					__pragma__ ('</all>')
				}
	...
	__nest__ (
		...
		'nacs.uris', {
			__all__: {
				__inited__: false,
				__init__: function (__all__) {
					var __name__ = 'nacs.uris';
					var uriThread = (function __lambda__ (tid) {
						return '/comments/' + tid;
					});
					var uriNewComment = (function __lambda__ (tid, cid) {
						return (uriThread (tid) + '/') + cid;
					});
					var uriAdmin = (function __lambda__ () {
						return '/admin';
					});
					var uriThreadSetting = (function __lambda__ (tid) {
						return '/threads/' + tid;
					});
					__pragma__ ('<all>')
						__all__.__name__ = __name__;
	...
	(function () {
		...
		var CommentCount = __init__ (__world__.nacs.count).CommentCount;
		var CommentArea = __init__ (__world__.nacs.comment).CommentArea;
		var main = function () {
			firebase.initializeApp (nacs_config);
			var __iterable0__ = $ ('.nacs-comment-area');
			for (var __index0__ = 0; __index0__ < len (__iterable0__); __index0__++) {
				var commentArea = __iterable0__ [__index0__];
				CommentArea ($ (commentArea).attr ('data-id'));
			}
			var __iterable0__ = $ ('.nacs-comment-count');
			for (var __index0__ = 0; __index0__ < len (__iterable0__); __index0__++) {
				var commentCount = __iterable0__ [__index0__];
				CommentCount ($ (commentCount).attr ('data-id'));
			}
		};
		$ (main);
		__pragma__ ('<use>' +
